...
## 2026-08-07 16:30 — 迭代 v4.67：统计预算卡已用/剩余显示实际金额 + 版本号 4.67.0

- 任务内容：
  - A. 修复统计「预算对比」卡底部摘要为占位文案的问题：原代码硬编码 `'已超出预算 ¥'` / `'已用 ¥ · 剩余 ¥'`（无金额），现改为显示实际金额（与首页/我的页预算卡一致）：超支显示 `已超出预算 ¥X`，未超支显示 `已用 ¥X · 剩余 ¥X`；同时修正该区块缩进错位。
  - B. 测试：新增组件测试「统计预算卡显示已用与剩余金额」（预算 100.00 / 已用 30.00 → 断言出现「已用 ¥30.00 · 剩余 ¥70.00」），163/163 通过。
  - C. 版本号 4.67.0+107；README/RELEASE/CHECKLIST/关于对话框同步。
- 修改文件：
  - `lib/pages/stats_page.dart`（预算卡摘要显示实际金额 + 缩进修正）
  - `lib/pages/profile_page.dart`（关于对话框更新日志加 v4.67）
  - `test/widget_test.dart`（新增回归测试）
  - `pubspec.yaml`、`README.md`、`RELEASE.md`、`CHECKLIST.md`
- commit hash：`f1e8cf9`；已 push（见下方状态）。
- 验证：`flutter analyze` 0 问题；`flutter test` 163/163（新增「统计预算卡显示已用与剩余金额」）。
- 遇到的问题与解决方案：stats_page 预算卡摘要此前误写为占位文案（v4.65 调整预算卡时遗留），已对照首页/我的页预算卡补齐实际金额；本次为纯 UI 文案修复，未重建 release APK。
- 下一步：首页今日模式预算显示，或「明细」按月导出；上架执行只差 Play 账号。