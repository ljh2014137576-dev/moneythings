...
class CsvExporter {
  ...
  static String exportCsv(List<Transaction> transactions) {
    final buf = StringBuffer('\uFEFF');
    buf.writeln('日期,类型,分类,金额(元),账户,备注');
    for (final t in transactions) {
      final d = t.date;
      final date = '${d.year}-${_p2(d.month)}-${_p2(d.day)} ${_p2(d.hour)}:${_p2(d.minute)}';
      ...
      buf.writeln('$date,$type,$category,$amount,$account,${_escape(t.note)}');
    }
    return buf.toString();
  }
